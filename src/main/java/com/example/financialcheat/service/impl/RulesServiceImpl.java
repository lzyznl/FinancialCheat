...
public class RulesServiceImpl extends ServiceImpl<RulesMapper, Rules>
    ...
    @Override
    public Map<String,List<projectDynamics>> projectDynamics(long projectId) {
        Map<String,List<projectDynamics>> rpdMap = new HashMap<>();
        List<projectDynamics> pdList = new ArrayList<>();
        ...
        List<Ruleshistory> projectDynamicsList = ruleshistoryService.query().eq("projectId", projectId).list();
        projectDynamicsList.forEach(ruleshistory -> {
            projectDynamics projectDynamics = new projectDynamics();
            projectDynamics.setEvent(ruleshistory.getEvent());
            projectDynamics.setTime(outputFormat1.format(ruleshistory.getCreateTime()));
            projectDynamics.setUsername(ruleshistory.getUserName());
            pdList.add(projectDynamics);
        });
        // 分组并存储到 Map
        pdList.forEach(projectDynamics -> {
            try {
                Date date = outputFormat1.parse(projectDynamics.getTime());
                String dateKey = outputFormat2.format(date);
                rpdMap.computeIfAbsent(dateKey, k -> new ArrayList<>()).add(projectDynamics);
